...
namespace Obsidian.VR
{
    public class VRUnitInputRelay : MonoBehaviour
    {
        ...
        {
            Vector2 move = _runtime.GetMoveVector();
            float throttle = _runtime.GetThrottle();

            Vector3 forward = _unit.transform.forward * throttle;
            Vector3 strafe = _unit.transform.right * move.x;
            Vector3 motion = forward + strafe;

            // ⭐ NEW — NavMesh movement instead of SetMoveInput
            Vector3 destination = _unit.transform.position + motion;
