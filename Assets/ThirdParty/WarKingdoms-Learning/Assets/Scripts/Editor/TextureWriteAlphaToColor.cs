...
public class TextureWriteAlphaToColor : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (running && texture != null)
        {
            Rect rect = new Rect(
                15f,
                200f,
                EditorGUIUtility.currentViewWidth - 30f,
                EditorGUIUtility.singleLineHeight * 2f);

            float progress =
                index / (float)(texture.width * texture.height);

            EditorGUI.ProgressBar(
                rect,
                progress,
                (progress * 100f).ToString("F3") + " %");
        }
    }
    ...
    private IEnumerator ChangeTexture()
    {
        ...
        // Use the instantiated texture directly with AssetDatabase.
        // GetEntityId() is not available on Texture2D in Unity 6.
